"""FastAPI server for TensorRT-LLM engine."""
...
async def initialize(request: InitializeRequest):
    ...
    try:
        ...
        if request.quantization:
            trt_kwargs["quantization"] = request.quantization
        
        # Add custom engine kwargs
        trt_kwargs.update(request.engine_kwargs)
...
async def generate(request: GenerateRequest):
    ...
    try:
        ...
        ttft = getattr(output, 'time_to_first_token', None)
...
if __name__ == "__main__":
    ...
    print(f"Starting TensorRT-LLM server on {args.host}:{args.port}")
